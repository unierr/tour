...
orgasm.startup = function () {
    ...
    rl.question('Choose path of pussy folder to cum there: ' + DECOR.fg.yellow, (answer) => {
    ...
    fs.readFile(files[0].path, function (err, data) {
        ...
        async.eachSeries(files, function iterator(file, callback) {
            if (file.type == 'directory') {
                ...
                fs.readFile('config.json', function (err, j) {
                    j = JSON.parse(j);
                    console.log('j is ', j)
                    j
                        .story
...
function scrape(path) {
    ...
    dirToJson(path, function (err, dirTree) {
        ...
        if (err) {
            ...
            scenesOf(dirTree.children, '', function (d) {
